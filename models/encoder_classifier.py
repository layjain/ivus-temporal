...
class EncoderClassifier2D(nn.Module):
    ...
    def infer_dims(self, args):
        in_sz = args.img_size
        dummy = torch.zeros(1, args.clip_len, 1, in_sz, in_sz).to(next(self.encoder.parameters()).device)
    ...
    def make_head(self, depth=1):
        ...
        mlp.add_module('classifier_FC', nn.Linear(self.args.mlp_encoding_size, 2))
        return mlp

    def forward(self, x):
        '''
        clip --encoder--> encoding ---average--> feats --FC--> Bx2 output 
        '''
        ...
        feats = self.selfsim_fc(feats) # B x 2
        return feats
